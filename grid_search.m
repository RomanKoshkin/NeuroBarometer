function [a_suc, u_suc] = grid_search(lambda, l_win_clas, en, fa)
...
%         plot(EEG.data(61,1000:1300), 'LineWidth', 3)
    ...
    a_suc = mean([S.a_success]);
    u_suc = mean([S.u_success]);
    disp(['Model accuracy (attended decoders): ' num2str(a_suc)])
    disp(['Model accuracy (unattended decoders): ' num2str(u_suc)])
end