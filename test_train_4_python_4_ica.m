...
EEG.icaact(1:2,:) = []; % remove the first components

CHAN_RANGE = 1:size(EEG.icaact,1);
...
for i = 1:length({S.type})
    ...
    while fin < S(i).latency + TRIAL_LEN*Fs
        counter = counter + 1;
        X(counter,:,:) = EEG.icaact(CHAN_RANGE,start:fin);
...
if WAVELET==0
    ...
    for i = 1:4 % windows:
        trial = i;
        subplot(4,1,i)
        plot(startfin(i,1):startfin(i,2), EEG.icaact(ch_cz, startfin(trial,1):startfin(trial,2)));hold on
